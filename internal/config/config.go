...
func Parse(args []string, output io.Writer) (Config, error) {
	...
	fs.StringVar(&cfg.OutputDir, "output-dir", "reports", "directory for timestamped JSON, five CSV files, and Markdown reports")
	...
	fs.Usage = func() {
		...
		fmt.Fprintln(output)
		fmt.Fprintln(output, "Discovers Autonomous Databases and Compute instances with OCI Search in every")
		fmt.Fprintln(output, "READY subscribed region, enriches them with Database, Compute, and Block Volume")
		fmt.Fprintln(output, "API details, and writes timestamped reports.")
		fmt.Fprintln(output)
		fs.PrintDefaults()
	}
